...
export async function generateMetadata(props: {
  ...
  return constructMetadata({
    title: isKo ? 'Vue 사용법' : 'Vue Usage',
    description: isKo
      ? 'Vue 3에서 jump-section 라이브러리를 사용하는 방법입니다. Composable과 컴포넌트 사용법을 알아보세요.'
      : 'How to use jump-section library in Vue 3. Learn about composables and components.',
    lang: params.lang,
    path: '/docs/vue',
  });
}
...
export default async function Page(props: { params: Promise<{ lang: Locale }> }) {
  ...
  const breadcrumbData = generateBreadcrumbJsonLd([
    ...
    { name: 'Vue', item: `https://jump-section.vercel.app/${params.lang}/docs/vue` },
  ]);

  const articleData = generateArticleJsonLd({
    title: isKo ? 'Vue 사용법' : 'Vue Usage',
    description: isKo
      ? 'Vue 3에서 jump-section 라이브러리를 사용하는 방법입니다.'
      : 'How to use jump-section library in Vue 3.',
    url: `https://jump-section.vercel.app/${params.lang}/docs/vue`,
    authorName: 'k_jin.0',
  });
